    ...
    def _record_to_stream(self, record, field_name):
        if request and request.params.get("download") and not self.env.su:
            ...
            if user.has_group("project.group_project_user") and not user.has_group(
                ...
                # If the record being downloaded is an attachment, get its parent target
                ...
                if target_model in ir_attachment._PROJECT_GUARDED_MODELS:
                    if not ir_attachment._is_allowed_project_attachment_target(
                        target_model, target_id, user
                    ):
                        raise AccessError(
                            _(
                                "You are not allowed to download attachments from project records where you are not the project responsible or task assignee."
                            )
                        )
