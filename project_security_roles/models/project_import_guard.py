    ...
    def _record_to_stream(self, record, field_name):
        if request and request.params.get('download'):
            ...
            if user.has_group("project.group_project_user") and not user.has_group("project.group_project_manager"):
                if record._name == 'ir.attachment' and record.res_model in record._PROJECT_GUARDED_MODELS:
                    if not record._is_allowed_project_attachment_target(record.res_model, record.res_id, user):
                        raise AccessError(
                            _("No tienes permiso para descargar archivos de este proyecto/tarea.")
                        )
