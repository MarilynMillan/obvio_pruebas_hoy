...
class IrBinary(models.AbstractModel):
    ...
    def _record_to_stream(self, record, field_name):
        if record._name == "ir.attachment" and request and hasattr(request, 'params') and request.params.get('download'):
            user = self.env.user
            if not self.env.su and user.has_group("project.group_project_user") and not user.has_group("project.group_project_manager"):
                ...
                if model_name in self.env["ir.attachment"]._PROJECT_GUARDED_MODELS:
                    if not self.env["ir.attachment"]._is_allowed_project_attachment_target(model_name, res_id, user):
                        ...
                        raise AccessError(_("You can view the document, but you are not allowed to download attachments on project records where you are not the project responsible or task assignee."))
