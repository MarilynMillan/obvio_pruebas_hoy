...
class IrBinary(models.AbstractModel):
    ...
    def _record_to_stream(self, record, field_name):
        ...
        if request and request.params.get("download") and record._name == "ir.attachment":
            user = self.env.user
            if user.has_group("project.group_project_user") and not user.has_group(
                "project.group_project_manager"
            ):
                ...
                if model_name in self.env["ir.attachment"]._PROJECT_GUARDED_MODELS:
                    if not self.env["ir.attachment"]._is_allowed_project_attachment_target(
                        ...
                        raise AccessError(
                            _(
                                "You cannot download attachments from project records where you are not the project responsible or task assignee."
                            )
                        )
        return res
