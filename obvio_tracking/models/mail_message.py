    ...
    def _action_done(self, feedback=False, attachment_ids=None):
        ...
        for activity in self:
            ...
            if activity.create_uid and activity.create_uid != self.env.user:
                creator_msg = f"<br/><small><b>Actividad creada por:</b> {activity.create_uid.name or 'Unknown'} el {create_time}</small>"

            messages.append(f"""
                <div style="color: #666666; border-left: 3px solid #ccc; padding-left: 10px;">
                    <small><i>✅ Tarea marcada como hecha por {self.env.user.name} el {now_time}</i></small>
                    {creator_msg}
                </div>
            """)
