...
class MailActivity(models.Model):
    ...
    def _action_done(self, feedback=False, attachment_ids=None):
        ...
        for activity in self:
            ...
            creator_text = f" (Creado originalmente por {creator_name})" if creator_name != user_name else ""

            messages.append(f"""
                <div style="font-size: 13px; color: #666666; border-left: 3px solid #28a745; padding-left: 10px;">
                    <small><i>✅ Actividad marcada como hecha por {user_name} el {now_time}{creator_text}</i></small>
                    <br/><small><b>📅 Creada el:</b> {create_time}</small>
                </div>
            """)
        ...
        for activity in activities_to_log:
            ...
            if changes:
                ...
                self.env['mail.message'].create({
                    'body': f"""
                        <div style="font-size: 13px; color:#555; border-left:3px solid #ffc107; padding-left:10px;">
                            <small>
                                <i>✎ Actividad editada por {user_name} el {current_time}{creator_text}</i>
                            </small>
                            <br/><small><b>📅 Creada el:</b> {create_time}</small>
        ...
        if not self._context.get('skip_cancel_log'):
            for activity in self:
                if activity.res_model == 'project.task':
                    ...
                    self.env['mail.message'].create({
                        'body': f"""
                            <div style="font-size: 13px; color: #666666; border-left: 3px solid #dc3545; padding-left: 10px;">
                                <small><i>❌ Actividad cancelada por {user_name} el {current_time}{creator_text}</i></small>
                                <br/><small><b>📅 Creada el:</b> {create_time}</small>
