...
class MailActivity(models.Model):
    ...
    def _action_done(self, feedback=False, attachment_ids=None):
        ...
        for activity in self:
            ...
            creator_info = f"<b>Creada originalmente por:</b> {creator_name} el {create_time}" if creator_name != current_user else f"<b>Creada por ti el:</b> {create_time}"

            messages.append(f"""
                <div style="font-family: sans-serif; font-size: 13px; color: #444; border-left: 3px solid #28a745; padding-left: 10px; margin-top: 10px; background-color: #f9fff9; padding: 10px; border-radius: 4px;">
                    <div style="margin-bottom: 5px;">
                        <small><i>{creator_info}</i></small>
                    </div>
                    <div>
                        <small><i>✅ <b>Marcada como Hecha</b> por {current_user} el {now_time}</i></small>
                    </div>
                </div>
            """)
        ...
        for activity in activities_to_log:
            ...
            if changes:
                ...
                self.env['mail.message'].create({
                    'body': f"""
                        <div style="font-family: sans-serif; font-size: 13px; color: #444; border-left: 3px solid #17a2b8; padding-left: 10px; margin-top: 10px; background-color: #f4faff; padding: 10px; border-radius: 4px;">
                            <div style="margin-bottom: 5px;">
                                <small><i>{creator_info}</i></small>
                            </div>
                            <div>
                                <small><i>✎ <b>Actividad editada</b> por {current_user} el {current_time}</i></small>
                            </div>
        ...
        if not self._context.get('skip_cancel_log'):
            for activity in self:
                if activity.res_model == 'project.task':
                    ...
                    self.env['mail.message'].create({
                        'body': f"""
                            <div style="font-family: sans-serif; font-size: 13px; color: #444; border-left: 3px solid #dc3545; padding-left: 10px; margin-top: 10px; background-color: #fff4f4; padding: 10px; border-radius: 4px;">
                                <div style="margin-bottom: 5px;">
                                    <small><i>{creator_info}</i></small>
                                </div>
                                <div>
                                    <small><i>❌ <b>Actividad cancelada</b> por {current_user} el {current_time}</i></small>
                                </div>
                                <div style="margin-top: 5px;">
                                    <b>Asunto:</b> {activity.summary or activity.activity_type_id.name}<br/>
                                    <span style="font-size: 0.9em;"><b>Nota:</b> {activity.note or 'Sin nota'}</span>
                                </div>
                            </div>
                        """,
                        'model': activity.res_model,
