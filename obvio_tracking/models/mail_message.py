...
class MailActivity(models.Model):
    ...
    def _action_done(self, feedback=False, attachment_ids=None):
        ...
        for activity in self:
            ...
            messages.append(f"""
                <div style="color: #555; border-left: 3px solid #ccc; padding-left: 10px; font-size: 0.9em;">
                    <small><i>✅ Actividad marcada como hecha por {self.env.user.name} el {now_time}</i></small><br/>
                    <small><b>Creado por:</b> {creator_name} el {create_time}</small>
                </div>
            """)
                    ...
                    self.env['mail.message'].create({
                        'body': f"""
                            <div style="color: #555; border-left: 3px solid #ccc; padding-left: 10px; font-size: 0.9em;">
                                ...
                                <br/><small><b>Creado por:</b> {creator_name} el {create_time}</small>
