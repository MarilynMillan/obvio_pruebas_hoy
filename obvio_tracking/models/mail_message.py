...
class MailActivity(models.Model):
    ...
    def _action_done(self, feedback=False, attachment_ids=None):
        ...
        for activity in self:
            ...
            messages.append(f"""
                <div style="color: #666666; border-left: 3px solid #28a745; padding-left: 10px; margin-bottom: 5px;">
                    <small><i>📅 Creada por {creator_name} el {create_time}</i></small><br/>
                    <small><i>✅ Finiquitada por {self.env.user.name} el {now_time}</i></small>
                </div>
            """)
        ...
        for activity in activities_to_log:
            ...
            if changes:
                ...
                self.env['mail.message'].create({
                    'body': f"""
                        <div style="color: #666666; border-left: 3px solid #ffc107; padding-left: 10px; margin-bottom: 5px;">
                            <small><i>📅 Creada por {creator_name} el {create_time}</i></small><br/>
        ...
        if not self._context.get('skip_cancel_log'):
            for activity in self:
                if activity.res_model == 'project.task':
                    ...
                    self.env['mail.message'].create({
                        'body': f"""
                            <div style="color: #666666; border-left: 3px solid #dc3545; padding-left: 10px; margin-bottom: 5px;">
                                <small><i>📅 Creada por {creator_name} el {create_time}</i></small><br/>
