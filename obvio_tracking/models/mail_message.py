...
class MailActivity(models.Model):
    ...
    def _action_done(self, feedback=False, attachment_ids=None):
        ...
        for activity in self:
            ...
            creator_name = activity.create_uid.name or 'Usuario'
            current_user_name = self.env.user.name or 'Usuario'

            messages.append(f"""
                <div style="color:#555; border-left:3px solid #28a745; padding-left:10px; margin-top: 5px;">
                    <div style="margin-bottom: 4px;">
                        <small><i>✅ Actividad marcada como hecha por <b>{current_user_name}</b> el {now_time}</i></small>
                    </div>
                    <div>
                        <small><i>📅 Creada originalmente por <b>{creator_name}</b> el {create_time}</i></small>
                    </div>
                </div>
            """)
        ...
        for activity in activities_to_log:
            ...
            if changes:
                ...
                self.env['mail.message'].create({
                    'body': f"""
                        <div style="color:#555; border-left:3px solid #ffc107; padding-left:10px; margin-top: 5px;">
                            <div style="margin-bottom: 4px;">
                                <small><i>✎ Actividad editada por <b>{current_user_name}</b> el {current_time}</i></small>
                            </div>
        ...
        if not self._context.get('skip_cancel_log'):
            for activity in self:
                if activity.res_model == 'project.task':
                    ...
                    self.env['mail.message'].create({
                        'body': f"""
                            <div style="color:#555; border-left:3px solid #dc3545; padding-left:10px; margin-top: 5px;">
                                <div style="margin-bottom: 4px;">
                                    <small><i>❌ Actividad cancelada por <b>{current_user_name}</b> el {current_time}</i></small>
                                </div>
                                <div style="margin-bottom: 6px;">
                                    <small><i>📅 Creada originalmente por <b>{creator_name}</b> el {create_time}</i></small>
                                </div>
                                <div><b>Asunto:</b> {activity.summary or activity.activity_type_id.name}</div>
                                <div style="font-size: 0.9em; margin-top: 4px;"><b>Nota:</b> {activity.note or 'Sin nota'}</div>
                            </div>
                        """,
                        'model': activity.res_model,
                        'res_id': activity.res_id,
