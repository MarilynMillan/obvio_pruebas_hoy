    ...
    def _action_done(self, feedback=False, attachment_ids=None):
        ...
        for activity in self:
            ...
            if activity.create_uid and activity.create_uid != self.env.user:
                creator = f" por {activity.create_uid.name}"

            messages.append(f"""
<div style="color: #666666; border-left: 3px solid #ccc; padding-left: 10px; margin-top: 5px;">
    <small>
        📅 <b>Creado el:</b> {create_time}{creator}<br/>
        ✅ <b>Terminado el:</b> {now_time} por {self.env.user.name}
    </small>
</div>
""")
                ...
                self.env['mail.message'].create({
                    'body': f"""
                        <div style="color: #666666; border-left: 3px solid #ccc; padding-left: 10px;">
                            <small>
                                📅 <b>Creado el:</b> {create_time}{creator}<br/>
                                ✎ <b>Editado el:</b> {current_time} por {self.env.user.name}
                            </small>
                    ...
                    self.env['mail.message'].create({
                        'body': f"""
                            <div style="color: #666666; border-left: 3px solid #ccc; padding-left: 10px;">
                                <small>
                                    📅 <b>Creado el:</b> {create_time}{creator}<br/>
                                    ❌ <b>Cancelado el:</b> {current_time} por {self.env.user.name}
                                </small>
