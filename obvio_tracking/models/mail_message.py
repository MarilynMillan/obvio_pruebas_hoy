...
class MailActivity(models.Model):
    ...
    def _action_done(self, feedback=False, attachment_ids=None):
        ...
        for activity in self:
            ...
            creator_name = getattr(activity.create_uid, 'name', 'Unknown')
            creator_html = ""
            # Only show creator info explicitly if the person finishing it is not the original creator
            if activity.create_uid and activity.create_uid != self.env.user:
                creator_html = f"""<br/><small><i>📅 Creada por {creator_name} el {create_time}</i></small>"""

            messages.append(f"""
                <div style="color: #555; border-left: 3px solid #6c757d; padding-left: 10px; margin-bottom: 5px;">
                    <small><i>✅ Actividad marcada como hecha por {self.env.user.name} el {now_time}</i></small>{creator_html}
                </div>
            """)
        ...
        for activity in activities_to_log:
            ...
            if changes:
                ...
                self.env['mail.message'].create({
                    'body': f"""
                        <div style="color: #555; border-left: 3px solid #6c757d; padding-left: 10px; margin-bottom: 5px;">
                            <small><i>✎ Actividad editada por {self.env.user.name} el {current_time}</i></small>{creator_html}
            ...
            for activity in self:
                if activity.res_model == 'project.task':
                    ...
                    self.env['mail.message'].create({
                        'body': f"""
                            <div style="color: #555; border-left: 3px solid #6c757d; padding-left: 10px; margin-bottom: 5px;">
                                <small><i>❌ Actividad cancelada por {self.env.user.name} el {current_time}</i></small>{creator_html}
