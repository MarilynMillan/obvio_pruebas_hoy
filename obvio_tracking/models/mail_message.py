...
class MailActivity(models.Model):
    ...
    def _action_done(self, feedback=False, attachment_ids=None):
        ...
        for activity in self:
            ...
            current_user_name = self.env.user.name or 'Desconocido'

            messages.append(f'''
<div style="color: #666666; border-left: 3px solid #28a745; padding-left: 10px;">
    <small>📅 Fecha de creación: {create_time} por {creator_name}</small><br/>
    <small>✅ Fecha en la que se marcó como hecha: {now_time} por {current_user_name}</small>
</div>
''')
        ...
        for activity in activities_to_log:
            ...
            if changes:
                ...
                self.env['mail.message'].create({
                    'body': f'''
<div style="color: #666666; border-left: 3px solid #6c757d; padding-left: 10px;">
    <small>📅 Fecha de creación: {create_time} por {creator_name}</small><br/>
    <small>✎ Fecha en la que se editó: {current_time} por {current_user_name}</small>
        ...
        if not self._context.get('skip_cancel_log'):
            for activity in self:
                if activity.res_model == 'project.task':
                    ...
                    self.env['mail.message'].create({
                        'body': f'''
<div style="color: #666666; border-left: 3px solid #dc3545; padding-left: 10px;">
    <small>📅 Fecha de creación: {create_time} por {creator_name}</small><br/>
    <small>🗙 Fecha en la que se canceló: {current_time} por {current_user_name}</small><br/>
    <span style="font-size: 0.9em;"><b>Asunto:</b> {activity.summary or activity.activity_type_id.name}</span><br/>
