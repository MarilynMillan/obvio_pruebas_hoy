    ...
    def _action_done(self, feedback=False, attachment_ids=None):
        ...
        for activity in self:
            ...
            if activity.create_uid and activity.create_uid != self.env.user:
                creator_str = f"<br/><small>👤 <b>Creado por:</b> {creator_name}</small>"

            messages.append(f"""
                <div style="color: #666666; border-left: 3px solid #28a745; padding-left: 10px; margin-bottom: 10px; font-size: 0.9em;">
                    <div style="font-weight: bold; margin-bottom: 5px; color: #28a745;">✅ Actividad Completada</div>
                    <small>👤 <b>Procesado por:</b> {current_user_name}</small>
                    {creator_str}
                    <br/><small>📅 <b>Fecha de Creación:</b> {create_time}</small>
                    <br/><small>🕒 <b>Fecha de Finalización:</b> {now_time}</small>
                </div>
            """)
        ...
        for activity in activities_to_log:
            ...
            if changes:
                ...
                self.env['mail.message'].create({
                    'body': f"""
                        <div style="color: #666666; border-left: 3px solid #ffc107; padding-left: 10px; margin-bottom: 10px; font-size: 0.9em;">
                            <div style="font-weight: bold; margin-bottom: 5px; color: #d39e00;">✎ Actividad Editada</div>
                            <small>👤 <b>Procesado por:</b> {current_user_name}</small>
                            {creator_str}
                            <br/><small>📅 <b>Fecha de Creación:</b> {create_time}</small>
                            <br/><small>🕒 <b>Fecha de Edición:</b> {current_time}</small>
                            <ul style="margin:6px 0 0 15px; padding:0; color:#555;">
                                {''.join(changes)}
                            </ul>
                        </div>
                    """,
        ...
        if not self._context.get('skip_cancel_log'):
            for activity in self:
                if activity.res_model == 'project.task':
                    ...
                    self.env['mail.message'].create({
                        'body': f"""
                            <div style="color: #666666; border-left: 3px solid #dc3545; padding-left: 10px; margin-bottom: 10px; font-size: 0.9em;">
                                <div style="font-weight: bold; margin-bottom: 5px; color: #dc3545;">❌ Actividad Cancelada</div>
                                <small>👤 <b>Procesado por:</b> {current_user_name}</small>
                                {creator_str}
                                <br/><small>📅 <b>Fecha de Creación:</b> {create_time}</small>
                                <br/><small>🕒 <b>Fecha de Cancelación:</b> {current_time}</small>
