...
class MailActivity(models.Model):
    ...
    def _action_done(self, feedback=False, attachment_ids=None):
        ...
        for activity in self:
            ...
            creator_name = activity.create_uid.name or 'Unknown' if activity.create_uid else 'Unknown'

            messages.append(f"""
                        <div style="color:#555; border-left:3px solid #6c757d; padding-left:10px;">
                            <small>
                                <i>✅ Actividad marcada como hecha por {self.env.user.name} el {now_time}</i>
                            </small>
                            <br/>
                            <small>
    ...
    def write(self, vals):
        ...
        for activity in activities_to_log:
            ...
            if changes:
                ...
                create_time = fields.Datetime.to_datetime(
                    activity.create_date
                ).astimezone(user_tz).strftime('%Y-%m-%d %H:%M:%S') if activity.create_date else 'Unknown'
                creator_name = activity.create_uid.name or 'Unknown' if activity.create_uid else 'Unknown'
                ...
                if activity.res_model == 'project.task':
                    ...
                    self.env['mail.message'].create({
                        'body': f"""
                            <div style="color:#555; border-left:3px solid #6c757d; padding-left:10px;">
                                <small>
                                    <i>❌ Actividad cancelada por {self.env.user.name} el {current_time}</i>
                                </small>
                                <br/>
                                <small>
                                    <i>Creada por {creator_name} el {create_time}</i>
