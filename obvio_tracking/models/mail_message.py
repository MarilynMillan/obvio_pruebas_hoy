...
class MailActivity(models.Model):
    ...
    def _action_done(self, feedback=False, attachment_ids=None):
        ...
        <div style="color:#555; border-left:3px solid #6c757d; padding-left:10px;">
            <small>
                <i>✅ Finalizado por {self.env.user.name or 'Desconocido'} el {now_time}</i><br/>
                <i>📅 Creado por {activity.create_uid.name or 'Desconocido'} el {create_time}</i>
            </small>
        </div>
    """)
            ...
            if changes:
                ...
                self.env['mail.message'].create({
                    'body': f"""
                        <div style="color:#555; border-left:3px solid #6c757d; padding-left:10px;">
                            <small>
                                <i>✎ Editado por {self.env.user.name or 'Desconocido'} el {current_time}</i><br/>
                                <i>📅 Creado por {activity.create_uid.name or 'Desconocido'} el {create_time}</i>
                            </small>
                ...
                if activity.res_model == 'project.task':
                    ...
                    self.env['mail.message'].create({
                        'body': f"""
                            <div style="color:#555; border-left:3px solid #6c757d; padding-left:10px;">
                                <small>
                                    <i>❌ Cancelado por {self.env.user.name or 'Desconocido'} el {current_time}</i><br/>
                                    <i>📅 Creado por {activity.create_uid.name or 'Desconocido'} el {create_time}</i>
                                </small>
