...
class MailActivity(models.Model):
    ...
    def _action_done(self, feedback=False, attachment_ids=None):
        ...
        for activity in self:
            ...
            creator_html = ""
            if activity.create_uid != self.env.user:
                creator = activity.create_uid.name or 'Desconocido'
                creator_html = f"<br/><small><b>📅 Creada originalmente por {creator} el {create_time}</b></small>"

            messages.append(f"""
<div style="color: #666666; border-left: 3px solid #ccc; padding-left: 10px; margin-top: 5px; background-color: #fcfcfc;">
    <small><i>✅ Actividad marcada como hecha por {self.env.user.name or 'Desconocido'} el {now_time}</i></small>
    {creator_html}
</div>
""")
        ...
        for activity in activities_to_log:
            ...
            if changes:
                ...
                self.env['mail.message'].create({
                    'body': f"""
<div style="color: #666666; border-left: 3px solid #ccc; padding-left: 10px; margin-top: 5px; background-color: #fcfcfc;">
    <small><i>✎ Actividad editada por {self.env.user.name or 'Desconocido'} el {current_time}</i></small>
    {creator_html}
            ...
            for activity in self:
                if activity.res_model == 'project.task':
                    ...
                    self.env['mail.message'].create({
                        'body': f"""
<div style="color: #666666; border-left: 3px solid #ccc; padding-left: 10px; margin-top: 5px; background-color: #fcfcfc;">
    <small><i>❌ Actividad cancelada por {self.env.user.name or 'Desconocido'} el {current_time}</i></small>
