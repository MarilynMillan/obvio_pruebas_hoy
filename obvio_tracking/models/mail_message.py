...
class MailActivity(models.Model):
    ...
    def _action_done(self, feedback=False, attachment_ids=None):
        ...
        for activity in self:
            ...
            creator_info = f" by {activity.create_uid.name}" if activity.create_uid and activity.create_uid != self.env.user else ""

            messages.append(f"""
                <div style="color: #666666; border-left: 3px solid #6c757d; padding-left: 10px; font-family: sans-serif; font-size: 13px; background-color: #fcfcfc; padding: 5px;">
                    <small><i>✅ Activity marked as done by {self.env.user.name} el {now_time}</i></small><br/>
                    <small><b>📅 Activity was created:</b> {create_time}{creator_info}</small>
                </div>
            """)
        ...
        for activity in activities_to_log:
            ...
            if changes:
                ...
                self.env['mail.message'].create({
                    'body': f"""
                        <div style="color: #666666; border-left: 3px solid #6c757d; padding-left: 10px; font-family: sans-serif; font-size: 13px; background-color: #fcfcfc; padding: 5px;">
                            <small><i>✎ Actividad editada por {self.env.user.name} el {current_time}</i></small><br/>
                            <small><b>📅 Activity was created:</b> {create_time}{creator_info}</small>
                    ...
                    self.env['mail.message'].create({
                        'body': f"""
                            <div style="color: #666666; border-left: 3px solid #6c757d; padding-left: 10px; font-family: sans-serif; font-size: 13px; background-color: #fcfcfc; padding: 5px;">
                                ...
                                <br/><small><b>📅 Activity was created:</b> {create_time}{creator_info}</small>
