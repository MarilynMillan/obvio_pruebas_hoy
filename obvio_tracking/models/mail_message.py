...
class MailActivity(models.Model):
    ...
    def _action_done(self, feedback=False, attachment_ids=None):
        ...
        for activity in self:
            ...
            ).astimezone(user_tz).strftime('%Y-%m-%d %H:%M:%S') if activity.create_date else 'Desconocida'

            creator_name = activity.create_uid.name or 'Desconocido'
            current_user = self.env.user.name or 'Desconocido'

            creator_info = f" Creada originalmente por {creator_name}." if activity.create_uid and activity.create_uid != self.env.user else ""

            msg = f"""
            <div style="color:#555; border-left:3px solid #28a745; padding-left:10px; margin-top:10px;">
                <small>
                    <i>✅ Actividad marcada como hecha por {current_user} el {now_time}.{creator_info}</i>
                </small>
    ...
    def write(self, vals):
        ...
        for activity in activities_to_log:
            ...
            if changes:
                ...
                create_time = fields.Datetime.to_datetime(activity.create_date).astimezone(user_tz).strftime('%Y-%m-%d %H:%M:%S') if activity.create_date else 'Desconocida'
                creator_name = activity.create_uid.name or 'Desconocido'
                current_user = self.env.user.name or 'Desconocido'
                creator_info = f" Creada originalmente por {creator_name}." if activity.create_uid and activity.create_uid != self.env.user else ""
                ...
                self.env['mail.message'].create({
                    'body': f"""
                        <div style="color:#555; border-left:3px solid #17a2b8; padding-left:10px; margin-top:10px;">
                            <small>
                                <i>✎ Actividad editada por {current_user} el {current_time}.{creator_info}</i>
                            </small>
                            <br/><small><b>📅 Fecha de creación:</b> {create_time}</small>
                            <ul style="margin:6px 0 0 15px; padding:0; font-size: 0.9em;">
                                {''.join(changes)}
                            </ul>
                        </div>
                    """,
        ...
        if not self._context.get('skip_cancel_log'):
            for activity in self:
                if activity.res_model == 'project.task':
                    ...
                    self.env['mail.message'].create({
                        'body': f"""
                            <div style="color:#555; border-left:3px solid #dc3545; padding-left:10px; margin-top:10px;">
                                <small>
                                    <i>❌ Actividad cancelada por {current_user} el {current_time}.{creator_info}</i>
                                </small>
                                <br/><small><b>📅 Fecha de creación:</b> {create_time}</small>
                                <br/><span style="font-size: 0.9em;"><b>Asunto:</b> {activity.summary or activity.activity_type_id.name}</span>
                                <br/><span style="font-size: 0.9em;"><b>Nota:</b> {activity.note or 'Sin nota'}</span>
                            </div>
                        """,
                        'model': activity.res_model,
                        'res_id': activity.res_id,
