...
class MailActivity(models.Model):
    ...
    def _action_done(self, feedback=False, attachment_ids=None):
        ...
        for activity in self:
            ...
            messages.append(f"""
<div style="color: #555; font-size: 13px; border-left: 3px solid #ccc; padding-left: 10px;">
    <div>📅 Creada por {creator_name} el {create_time}</div>
    <div>✅ Completada por {self.env.user.name} el {now_time}</div>
</div>
""")
            ...
            if changes:
                ...
                self.env['mail.message'].create({
                    'body': f"""
                        <div style="color: #555; font-size: 13px; border-left: 3px solid #ccc; padding-left: 10px;">
                            <div>📅 Creada por {creator_name} el {create_time}</div>
                            <div>✎ Editada por {self.env.user.name} el {current_time}</div>
                ...
                if activity.res_model == 'project.task':
                    ...
                    self.env['mail.message'].create({
                        'body': f"""
                            <div style="color: #555; font-size: 13px; border-left: 3px solid #ccc; padding-left: 10px;">
                                <div>📅 Creada por {creator_name} el {create_time}</div>
                                <div>❌ Cancelada por {self.env.user.name} el {current_time}</div>
                                <div style="margin-top: 4px;"><b>Asunto:</b> {activity.summary or activity.activity_type_id.name}</div>
                                <div><span style="font-size: 0.9em;">Nota: {activity.note or 'Sin nota'}</span></div>
                            </div>
                        """,
                        'model': activity.res_model,
                        'res_id': activity.res_id,
