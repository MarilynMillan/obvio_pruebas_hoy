    ...
    def _action_done(self, feedback=False, attachment_ids=None):
        ...
        for activity in self:
            ...
            if activity.create_uid and activity.create_uid != self.env.user:
                ...
                creator_str = f" - Creada por {creator_name} el {create_time}"

            user_name = self.env.user.name or 'Desconocido'

            messages.append(f"""
                <div style="color:#555; border-left:3px solid #6c757d; padding-left:10px;">
                    <small><i>✅ Actividad marcada como hecha por {user_name} el {now_time}{creator_str}</i></small>
                </div>
            """)
            ...
            if changes:
                ...
                self.env['mail.message'].create({
                    'body': f"""
                        <div style="color:#555; border-left:3px solid #6c757d; padding-left:10px;">
                            <small>
                                <i>✎ Actividad editada por {user_name} el {current_time}{creator_str}</i>
                            </small>
            ...
            for activity in self:
                if activity.res_model == 'project.task':
                    ...
                    self.env['mail.message'].create({
                        'body': f"""
                            <div style="color:#555; border-left:3px solid #6c757d; padding-left:10px;">
                                <small><i>❌ Actividad cancelada por {user_name} el {current_time}{creator_str}</i></small>
