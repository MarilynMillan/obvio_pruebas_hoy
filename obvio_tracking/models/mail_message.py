        ...
        for activity in self:
            ...
            activity_html = f"""
                <div style="color:#555; border-left:3px solid #6c757d; padding-left:10px;">
                    <small style="display: block; margin-bottom: 2px;">
                        📅 <i>Creada por {creator_name} el {create_time}</i>
                    </small>
                    <small style="display: block;">
                        ✅ <i>Finalizada por {current_user_name} el {now_time}</i>
                    </small>
                </div>
            """
            messages.append(activity_html)

        if feedback:
            messages.insert(0, f"<div style='margin-bottom: 5px;'>{feedback}</div>")
                ...
                self.env['mail.message'].create({
                    'body': f"""
                        <div style="color:#555; border-left:3px solid #6c757d; padding-left:10px;">
                            ...
                            <small style="display: block;">
                                ✎ <i>Editada por {current_user_name} el {current_time}</i>
                            </small>
                        ...
                        'body': f"""
                            <div style="color:#555; border-left:3px solid #6c757d; padding-left:10px;">
                                ...
                                <small style="display: block; margin-bottom: 5px;">
                                    🗙 <i>Cancelada por {current_user_name} el {current_time}</i>
                                </small>
                                <div><b>Asunto:</b> {activity.summary or activity.activity_type_id.name}</div>
                                <div style="font-size: 0.9em; margin-top: 3px;">Nota: {activity.note or 'Sin nota'}</div>
                            </div>
                        """,
                        'model': activity.res_model,
                        'res_id': activity.res_id,
