...
class MailActivity(models.Model):
    ...
    def _action_done(self, feedback=False, attachment_ids=None):
        ...
        for activity in self:
            ...
            messages.append(f'''
    <div style="color: #4CAF50; border-left: 3px solid #4CAF50; padding-left: 10px; margin-top: 10px; font-family: sans-serif;">
        <div style="font-weight: bold; margin-bottom: 5px;">✅ Actividad marcada como hecha</div>
        <small style="color: #666;">
            <b>Creada por:</b> {creator_name} el {create_time}<br/>
            <b>Finalizada por:</b> {self.env.user.name} el {now_time}
        </small>
    </div>
            ''')
        ...
        for activity in activities_to_log:
            ...
            if changes:
                ...
                self.env['mail.message'].create({
                    'body': f'''
                        <div style="color: #555; border-left: 3px solid #007bff; padding-left: 10px; margin-top: 10px; font-family: sans-serif;">
                            <div style="font-weight: bold; margin-bottom: 5px; color: #007bff;">✎ Actividad editada</div>
                            <small style="color: #666;">
                                <b>Creada por:</b> {creator_name} el {create_time}<br/>
                                <b>Editada por:</b> {self.env.user.name} el {current_time}
                            </small>
        ...
        if not self._context.get('skip_cancel_log'):
            for activity in self:
                if activity.res_model == 'project.task':
                    ...
                    self.env['mail.message'].create({
                        'body': f'''
                            <div style="color: #a94442; border-left: 3px solid #d9534f; padding-left: 10px; margin-top: 10px; font-family: sans-serif;">
                                <div style="font-weight: bold; margin-bottom: 5px; color: #d9534f;">❌ Actividad cancelada</div>
                                <small style="color: #666;">
                                    <b>Creada por:</b> {creator_name} el {create_time}<br/>
                                    <b>Cancelada por:</b> {self.env.user.name} el {current_time}
                                </small>
                                <div style="margin-top: 10px; padding: 5px; background-color: #f9f2f2; border-radius: 3px;">
