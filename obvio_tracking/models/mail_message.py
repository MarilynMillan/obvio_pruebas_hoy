    ...
    def _action_done(self, feedback=False, attachment_ids=None):
        ...
        for activity in self:
            ...
            if activity.create_uid and activity.create_uid.id != self.env.user.id:
                ...
                creator_text = f" (por {creator_name})"

            messages.append(f"""
                <div style="color: #666666; border-left: 3px solid #ccc; padding-left: 10px; margin-top: 10px;">
                    <small><i>✅ Actividad Completada por {self.env.user.name or 'Unknown'} el {now_time}</i></small>
                    <br/><small><b>📅 Creada:</b> {create_time}{creator_text}</small>
                </div>
            """)
        ...
        for activity in activities_to_log:
            ...
            if changes:
                ...
                self.env['mail.message'].create({
                    'body': f"""
                        <div style="color: #666666; border-left: 3px solid #ccc; padding-left: 10px; margin-top: 10px;">
                            <small><i>✎ Actividad Editada por {self.env.user.name or 'Unknown'} el {current_time}</i></small>
                            <br/><small><b>📅 Creada:</b> {create_time}{creator_text}</small>
            ...
            for activity in self:
                if activity.res_model == 'project.task':
                    ...
                    self.env['mail.message'].create({
                        'body': f"""
                            <div style="color: #666666; border-left: 3px solid #ccc; padding-left: 10px; margin-top: 10px;">
                                <small><i>❌ Actividad Cancelada por {self.env.user.name or 'Unknown'} el {current_time}</i></small>
