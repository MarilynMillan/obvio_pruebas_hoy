...
class MailActivity(models.Model):
    ...
    def _action_done(self, feedback=False, attachment_ids=None):
        ...
        for activity in self:
            ...
            messages.append(f"""
    <div style="color: #666666; border-left: 3px solid #ccc; padding-left: 10px; font-family: sans-serif;">
        <small><i>📅 Creada por {activity.create_uid.name or 'Desconocido'} el {create_time}</i></small><br/>
        <small><i>✅ Marcada como hecha por {self.env.user.name} el {now_time}</i></small>
    </div>
    """)
        ...
        for activity in activities_to_log:
            ...
            if changes:
                ...
                self.env['mail.message'].create({
                    'body': f"""
                        <div style="color: #666666; border-left: 3px solid #ccc; padding-left: 10px; font-family: sans-serif;">
                            <small><i>📅 Creada por {activity.create_uid.name or 'Desconocido'} el {fields.Datetime.to_datetime(activity.create_date).astimezone(user_tz).strftime('%Y-%m-%d %H:%M:%S')}</i></small><br/>
