...
class MailActivity(models.Model):
    ...
    def _action_done(self, feedback=False, attachment_ids=None):
        ...
        for activity in self:
            ...
            doer_name = self.env.user.name or 'Desconocido'

            messages.append(f"""
                <div style="color: #666666; border-left: 3px solid #ccc; padding-left: 10px; margin-top: 5px; margin-bottom: 5px;">
                    <small><i>✅ Actividad marcada como hecha por {doer_name} el {now_time}</i></small>
                    <br/><small><b>Creada originalmente por {creator_name} el:</b> {create_time}</small>
                </div>
            """)
            ...
            if changes:
                ...
                self.env['mail.message'].create({
                    'body': f"""
                        <div style="color: #666666; border-left: 3px solid #ccc; padding-left: 10px; margin-top: 5px; margin-bottom: 5px;">
                            <small>
                                <i>✎ Actividad editada por {doer_name} el {current_time}</i>
                            </small>
                            <br/><small><b>Creada originalmente por {creator_name} el:</b> {create_time}</small>
            ...
            for activity in self:
                if activity.res_model == 'project.task':
                    ...
                    self.env['mail.message'].create({
                        'body': f"""
                            <div style="color: #666666; border-left: 3px solid #ccc; padding-left: 10px; margin-top: 5px; margin-bottom: 5px;">
                                <small><i>🗙 Actividad cancelada por {doer_name} el {current_time}</i></small>
                                <br/><small><b>Creada originalmente por {creator_name} el:</b> {create_time}</small>
