...
class MailActivity(models.Model):
    ...
    def _action_done(self, feedback=False, attachment_ids=None):
        ...
        for activity in self:
            ...
            creator_info = f"👤 Creada por {creator_name} el {create_time}" if creator != current_user else f"📅 Creada el {create_time}"

            messages.append(f"""
                <div style="font-family: inherit; color:#555; border-left:3px solid #6c757d; padding-left:10px; margin-top: 5px;">
                    <small>
                        <i>✅ Actividad marcada como hecha por {current_user.name} el {now_time}</i>
                        <br/><span style="color: #888;">{creator_info}</span>
                    </small>
                </div>
            """)
            ...
            if changes:
                ...
                self.env['mail.message'].create({
                    'body': f"""
                        <div style="font-family: inherit; color:#555; border-left:3px solid #6c757d; padding-left:10px; margin-top: 5px;">
                            <small>
                                <i>✎ Actividad editada por {current_user.name} el {current_time}</i>
                                <br/><span style="color: #888;">{creator_info}</span>
                            </small>
                ...
                if activity.res_model == 'project.task':
                    ...
                    self.env['mail.message'].create({
                        'body': f"""
                            <div style="font-family: inherit; color:#555; border-left:3px solid #6c757d; padding-left:10px; margin-top: 5px;">
                                <small>
                                    <i>❌ Actividad cancelada por {current_user.name} el {current_time}</i>
                                    <br/><span style="color: #888;">{creator_info}</span>
                                </small>
