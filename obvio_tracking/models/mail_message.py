        ...
        for activity in self:
            ...
            messages.append(f'''
                <div style="color:#555; border-left:3px solid #6c757d; padding-left:10px;">
                    <small>
                        <i>✅ Actividad marcada como hecha el {now_time} por {self.env.user.name}</i>
                    </small>
                    <ul style="margin:6px 0 0 15px; padding:0; list-style-type: none;">
                        <li><b>📅 Creada:</b> {create_time}{creator_str}</li>
                    </ul>
                </div>
            ''')
            ...
            if changes:
                ...
                self.env['mail.message'].create({
                    'body': f'''
                        <div style="color:#555; border-left:3px solid #6c757d; padding-left:10px;">
                            <small>
                                <i>✎ Actividad editada el {current_time} por {self.env.user.name}</i>
                            </small>
                            <ul style="margin:6px 0 0 15px; padding:0; list-style-type: none;">
                                <li><b>📅 Creada:</b> {create_time}{creator_str}</li>
                                {''.join(changes)}
                ...
                if activity.res_model == 'project.task':
                    ...
                    self.env['mail.message'].create({
                        'body': f'''
                            <div style="color:#555; border-left:3px solid #6c757d; padding-left:10px;">
                                <small>
                                    <i>❌ Actividad cancelada el {current_time} por {self.env.user.name}</i>
                                </small>
                                <ul style="margin:6px 0 0 15px; padding:0; list-style-type: none;">
                                    <li><b>📅 Creada:</b> {create_time}{creator_str}</li>
                                    <li><b>Asunto:</b> {activity.summary or activity.activity_type_id.name}</li>
                                    <li><b>Nota:</b> <span style="font-size: 0.9em;">{activity.note or 'Sin nota'}</span></li>
                                </ul>
                            </div>
                        ''',
                        'model': activity.res_model,
