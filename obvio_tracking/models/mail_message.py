...
class MailActivity(models.Model):
    ...
    def _action_done(self, feedback=False, attachment_ids=None):
        ...
        for activity in self:
            ...
            creator = activity.create_uid
            creator_name = creator.name if creator else 'Desconocido'
            creator_info = f" por {creator_name}" if creator and creator != self.env.user else ""

            messages.append(f"""
                <div style="font-size: 13px; color: #555; border-left: 3px solid #28a745; padding-left: 10px; margin-top: 10px; background-color: #f8f9fa; padding-top: 5px; padding-bottom: 5px;">
                    <div style="margin-bottom: 4px;">📅 <b style="color: #333;">Fecha de creación:</b> {create_time}{creator_info}</div>
                    <div>✅ <b style="color: #333;">Marcada como hecha:</b> {now_time} por {self.env.user.name}</div>
                </div>
            """)
        ...
        for activity in activities_to_log:
            ...
            if changes:
                ...
                self.env['mail.message'].create({
                    'body': f"""
                        <div style="font-size: 13px; color:#555; border-left:3px solid #17a2b8; padding-left:10px; margin-top: 10px; background-color: #f8f9fa; padding-top: 5px; padding-bottom: 5px;">
                            <div style="margin-bottom: 4px;">📅 <b style="color: #333;">Fecha de creación:</b> {create_time}{creator_info}</div>
                            <div style="margin-bottom: 8px;">✎ <b style="color: #333;">Actividad editada:</b> {current_time} por {self.env.user.name}</div>
        ...
        if not self._context.get('skip_cancel_log'):
            for activity in self:
                if activity.res_model == 'project.task':
                    ...
                    self.env['mail.message'].create({
                        'body': f"""
                            <div style="font-size: 13px; color: #555; border-left: 3px solid #dc3545; padding-left: 10px; margin-top: 10px; background-color: #f8f9fa; padding-top: 5px; padding-bottom: 5px;">
                                <div style="margin-bottom: 4px;">📅 <b style="color: #333;">Fecha de creación:</b> {create_time}{creator_info}</div>
                                <div style="margin-bottom: 8px;">❌ <b style="color: #333;">Actividad cancelada:</b> {current_time} por {self.env.user.name}</div>
                                <div><b>Asunto:</b> {activity.summary or activity.activity_type_id.name}</div>
                                <div><b>Nota:</b> {activity.note or 'Sin nota'}</div>
                            </div>
                        """,
                        'model': activity.res_model,
                        'res_id': activity.res_id,
