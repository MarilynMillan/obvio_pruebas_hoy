...
class MailActivity(models.Model):
    ...
    def _action_done(self, feedback=False, attachment_ids=None):
        ...
        for activity in self:
            ...
            # Use same layout as write/unlink
            # Add creator notice to title if done by someone else
            action_user = self.env.user.name
            if self.env.user != activity.create_uid:
                title_suffix = f" (de {creator_name})"
            else:
                title_suffix = ""

            messages.append(f"""
                <div style="color:#555; border-left:3px solid #6c757d; padding-left:10px;">
                    <small><i>✅ Actividad marcada como hecha por {action_user}{title_suffix} el {now_time}</i></small>
                    <br/>
            ...
            if changes:
                ...
                self.env['mail.message'].create({
                    'body': f"""
                        <div style="color:#555; border-left:3px solid #6c757d; padding-left:10px;">
                            <small>
                                <i>✎ Actividad editada por {action_user}{title_suffix} el {current_time}</i>
                            </small>
                            <br/>
                            <small><b>📅 Creada por:</b> {creator_name} el {create_time}</small>
            ...
            for activity in self:
                if activity.res_model == 'project.task':
                    ...
                    self.env['mail.message'].create({
                        'body': f"""
                            <div style="color:#555; border-left:3px solid #6c757d; padding-left:10px;">
                                <small><i>❌ Actividad cancelada por {action_user}{title_suffix} el {current_time}</i></small>
                                <br/>
                                <small><b>📅 Creada por:</b> {creator_name} el {create_time}</small>
